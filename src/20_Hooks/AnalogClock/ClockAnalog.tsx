...
export const Clock = () => {
    ...
    useEffect(() => {
        const timerID = setInterval(() => setState(new Date()), 1000);
        return () => {
    ...
    {
        const hoursDegrees = state.getHours() * 30 + state.getMinutes() / 2;
        const minutesDegrees = state.getMinutes() * 6 + state.getSeconds() / 10;
        const secondsDegrees = state.getSeconds() * 6;
        ...
        return (
            <div>
                <div className={`${s.clock_container} ${s.styling}`}>
                    <div id="clock" className={`${s.clock_content}`}>

                        <svg className={`${s.background_numbers}`} viewBox="0 0 226.6 233.8">
